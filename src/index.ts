...
import { OriginalUser, JsonResult, UserRow, Range} from './types.js';

//Seteando  el sistema de archivo para XLSX
XLSX.set_fs(fs);

const workbookName: string  = 'Reporte General - EDIC.xlsx';
const worksheetName: string  = 'Reporte General';
const firstSurnameXlsxHeader: string = 'Apellido Paterno';
const lastSurnameXlsxHeader: string = 'Apellido Materno'
const firstnameXlsxHeader: string  = 'Nombre (s)';
const emailXlsxHeader: string = 'Correo electrónico';
const resultsXlsxHeader: string  = 'Resultado'
...
// Arreglo de rutas
const pathsArray : Array<string> = directoryContent.map( json => path.join(jsonsDirectoryPath, json))
// Arreglo de resultados JSON
const jsonResults : Array<JsonResult> = pathsArray.map( path => require(path) as JsonResult);


jsonResults.forEach(json => json.ranges.forEach(range => { if(range.score < 3 ) range.score = range.score + 1 }))
...
const originalUsersArr : Array<OriginalUser> = [];

for (const address in sheet) {
    //console.log(address);
    if(address.includes('C')) {
        const _completeName : string = sheet[address].v
...
const lowerFullNames : Array<Array<string>> = originalUsersArr.map(user => user.fullName.map(name => name.toLowerCase()).map(e => e.normalize("NFD").replace(/[\u0300-\u036f]/g, "")));
...
const userRows : Array<UserRow> = jsonResults.map(json => {

    const name : string = json.username.replace('@edicmexico.onmicrosoft.com','');
...
let userRowAoA : Array<Array<string | number>> = userRows.map( user => {
